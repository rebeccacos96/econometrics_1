...
Y(0,i) = no/not a postitive response from the host/not being accepteded to be a guest
Y(1,i) = yes/a postive response/ being accepted to be a guest
...
Effect Size: 0.05/0.5 = 0.1 - effect of 0.1 standard deviations is considered small.
...
6. Estimate the treatment effect. Replicate the three columns of Edelman and Luca 2017, Table 2, p. 8 (shown below). Use a linear probability model. Do not worry about clustering standard errors, we will get back to this later in this course. To show the standard errors rather than the confidence intervals, set the option confint=FALSE when using the command summ.
...
The regression analysis indicates a lower acceptance rate for those who are black/african-american as opposed to those who are not. The first two regression report an estimated change of -0.08 in acceptances which is bigger than the 5% minimum effect rate that was included earlier, therefore supporting the conclusion of the regression.
